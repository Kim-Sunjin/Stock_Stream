...
st.title("📈 나의 주식 포트폴리오 & 기술적 분석 대시보드")
...
def get_company_names(tickers):
    ...
    for ticker in tickers:
        try:
            query_ticker = str(ticker) + '.KS' if str(ticker).isdigit() else ticker
            info = yf.Ticker(query_ticker).info
            name = info.get('shortName', info.get('longName', str(ticker)))
            mapping[ticker] = name if name else str(ticker)
        except Exception:
            mapping[ticker] = str(ticker)
    return mapping
...
if uploaded_file is not None:
    ...
    selected_date = st.slider(
        ...
        min_value=min_date, max_value=max_date, value=max_date, format="YYYY-MM-DD"
    )
    ...
    with col_pie:
        ...
        if not df_snapshot.empty:
            fig_pie = go.Figure(data=[go.Pie(
                labels=df_snapshot['종목명'], values=df_snapshot['보유 금액(KRW)'], hole=.3, textinfo='label+percent'
            )])
    ...
    if not df_snapshot.empty:
        styled_snapshot_df = df_snapshot.style.format({
            '매수 단가(KRW)': '{:,.0f} 원', '보유 수량': '{:,.2f}', '보유 금액(KRW)': '{:,.0f} 원',
            '수익금(KRW)': '{:,.0f} 원', '수익률 (%)': '{:,.2f}%'
        })
    ...
    if not df_realized.empty:
        styled_df = df_realized.style.format({
            '매도 단가(KRW)': '{:,.0f} 원', '매수 평단가(KRW)': '{:,.0f} 원',
            '실현 수익금(KRW)': '{:,.0f} 원', '수익률 (%)': '{:,.2f}%'
        })
